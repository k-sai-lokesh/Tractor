...
export default function ListingsClient({ initialTractors }: { initialTractors: any[] }) {
  ...
  const filtered = useMemo(() => {
    let list = initialTractors.filter(t => {
      // Handle the fact that 'images' is a JSON string from database
      const images = typeof t.images === "string" ? JSON.parse(t.images) : t.images;
      t.images = images; // Update in place for convenient mapping later

      if (searchQ && !`${t.brand} ${t.model} ${t.location} ${t.owner?.name || ""}`.toLowerCase().includes(searchQ.toLowerCase())) return false;
    ...
    return list;
  }, [initialTractors, searchQ, selectedBrand, selectedType, maxPrice, minRating, availableOnly, sortBy]);
