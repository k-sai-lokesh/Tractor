...
export default function TractorDetailClient({ tractor }: { tractor: any }) {
  const router = useRouter();
  const { setBooking } = useBooking();

  // Parse images if they are stringified
  const images = typeof tractor.images === "string" ? JSON.parse(tractor.images) : (tractor.images || ["/tractor1.png"]);
  ...
  const attachmentTotalPerHour = tractor.attachments
    .filter((a: any) => selectedAttachments.includes(a.name))
    .reduce((sum: number, a: any) => sum + a.pricePerHour, 0);
  ...
  const handleBookNow = () => {
    setBooking({
      ...
      ownerName: tractor.owner?.name || "Unknown Owner",
      ownerMobile: tractor.owner?.mobile || "Privacy Protected",
      ownerUpiId: tractor.owner?.upiId || "",
      ...
      hours,
      days,
      selectedAttachments: tractor.attachments.filter((a: any) => selectedAttachments.includes(a.name)),
  ...
  const availableDays = ["Mon", "Tue", "Wed", "Thu", "Fri"]; // Mock fallback since it's not in schema yet
        ...
        <div className="container">
          <div style={{ display: "grid", gap: "1.5rem" }} className="lg:grid-cols-[1fr_auto]">
            ...
            <div style={{ position: "relative", height: "clamp(250px, 50vh, 480px)", borderRadius: "1.5rem", overflow: "hidden", background: "#0D2119", boxShadow: "0 20px 40px rgba(0,0,0,0.4)", border: "1px solid rgba(255,255,255,0.1)" }}>
              <Image src={images[activeImage]} alt={tractor.model} fill sizes="(max-width: 768px) 100vw, 800px" style={{ objectFit: "cover" }} priority />
          ...
          <div className="fade-in-up">
            ...
            <div style={{ background: "white", borderRadius: "1.5rem", padding: "2rem", border: "1px solid #E5E7EB", marginBottom: "2rem", boxShadow: "0 4px 12px rgba(0,0,0,0.03)" }}>
              ...
              <div style={{ display: "grid", gap: "1rem" }} className="sm:grid-cols-2">
                {[
                  ...
                  { label: "Working Days", value: availableDays.join(", "), icon: Calendar },
            ...
            <div style={{ background: "white", borderRadius: "1.5rem", padding: "2rem", border: "1px solid #E5E7EB", marginBottom: "2rem", boxShadow: "0 4px 12px rgba(0,0,0,0.03)" }}>
              ...
              <div className="grid-responsive grid-responsive-2" style={{ gap: "1rem" }}>
                {tractor.attachments.map((att: any) => (
            ...
            <div style={{ background: "white", borderRadius: "1.5rem", padding: "2rem", border: "1px solid #E5E7EB", boxShadow: "0 4px 12px rgba(0,0,0,0.03)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                {tractor.reviews.map((review: any) => (
                  <div key={review.id} style={{ padding: "1.5rem", background: "#F9FAFB", borderRadius: "1.25rem", border: "1px solid #F3F4F6" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.75rem", alignItems: "center" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                        <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#2D6A4F", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: "0.8rem" }}>{review.renterName[0]}</div>
                        <span style={{ fontWeight: 700, fontSize: "0.95rem", color: "#1B4332" }}>{review.renterName}</span>
                      </div>
                      <div style={{ display: "flex", gap: "0.1rem" }}>
            ...
            <div style={{ background: "white", borderRadius: "1.5rem", padding: "1.5rem", border: "1px solid #E5E7EB", boxShadow: "0 4px 12px rgba(0,0,0,0.03)" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.5rem" }}>
                <div style={{ width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg, #1B4332, #214E3A)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, color: "white", fontSize: "1.25rem", boxShadow: "0 4px 12px rgba(27,67,50,0.2)" }}>
                  {tractor.owner?.name?.split(" ").map((n: string) => n[0]).join("").slice(0,2) || "O"}
                </div>
                <div>
                  <p style={{ fontWeight: 800, color: "#111827", fontSize: "1.1rem" }}>{tractor.owner?.name || "Unknown Owner"}</p>
